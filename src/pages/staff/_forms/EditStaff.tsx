...
import { getClubById, getCluub, getFonction, getRoles, getStaffById } from '../../../requests';
...
const FormTeam = () => {
	...
	const [getStaffByIdU, setGetStaffByIdU] = useState<Staff | undefined>();

	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios.get(
					'https://spring-boot-sokker.onrender.com/api/roles/',
				);
	...
	useEffect(() => {
		getStaffById(Number(id)).then((res) => {
			...
			console.log(res.date_naissance);
		});
	}, [id]);
				...
				<SubHeaderLeft>
					<Breadcrumb
						list={[
							...
							{ title: 'Modifier staff', to: '/staff/modifier' },
						]}
					/>
				</SubHeaderLeft>
				<SubHeaderRight>
					...
					<Card>
						<CardHeader className='mt-4'>
							<CardLabel icon='person' iconColor='info'>
								<CardTitle>Modification Staff</CardTitle>
							</CardLabel>
						</CardHeader>
						<CardBody>
