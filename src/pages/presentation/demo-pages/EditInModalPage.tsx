...
const EditInModalPage = () => {
	...
	const [clubs, setClubs] = useState<datos[] | undefined>(undefined);
					...
					<Popovers
						desc={
							<DatePicker
								...
								date={date}
								color={import.meta.env.VITE_REACT_APP_PRIMARY_COLOR}
							/>
						}
						...
						<Card>
							<CardHeader borderSize={1}>
								...
								<CardActions>
									...
									<Button color='info' icon='add' isLight tag='a' to='ajouter'>
										Ajouter équipe
									</Button>
								</CardActions>
							</CardHeader>
										...
										{clubs
											? clubs.map((item, i) => (
													<tr key={item.id}>
														...
														<td>{item.genre}</td>
														<td>{item.categorieAge}</td>
														<td className='text-center'>
															{item.nbrJoueurs}
														</td>
														<td>
															{item.status ? 'approved' : 'canceled'}
														</td>
														{/* <td>
