...
export const formatDataList = <DataType>(
	data: GetData<DataType> | undefined,
	dataKeyToDisplay: string[],
	idKey: string,
) => {
	return data?.content?.map((dataList) => {
		return dataKeyToDisplay.map((dataKey) => {
			if (dataKey === 'status') {
				return {
					id: dataList[idKey],
					value: dataList[dataKey] ? 'Active' : 'Inactive',
					type: 'text',
				} as DataTableRow;
			}
			return {
				id: dataList[idKey],
				value: dataList[dataKey],
				type: dataKey === 'logo' || dataKey === 'photo' ? 'img' : 'text',
			} as DataTableRow;
		});
	});
};
